...
`flux1`, `flux2`, `z_image`, `fibo`, `qwen` (`qwen_image` in registry), `seedvr2`, `ltx`, `ltx25`, `wan`, `ace_step`, … — source of truth is `_transformer_registry.py` + `models_registry.json`.
...
- **MiniMax H3 (FL2VA)** — `video_pipeline_shape=family_generator` under `backend/engine/families/minimax_h3/`. **PipeNetwork/minimax-h3-mlx** is the parity reference for **DiT module tree, weights, scheduler, RoPE, and keyframe conditioning**. Default installs: **PipeNetwork** sharded DiT (`pipenetwork/MiniMax-H3-MLX-{4,8}bit`) + **MiniMaxAI/MiniMax-H3** `FL2VA/` aux (VAE / text encoder / tokenizer). **Compute-bound**: mlx-q4/q8 saves RAM more than wall time. Turbo: install LoRA **`minimax-h3-turbo-lora:v4-ema`** (larryvrh v4-600 EMA) or set **`h3_quality_preset=draft`** / **`h3_turbo=true`**; 4–8 steps (6–8 recommended). `h3_turbo` conflicts with TeaCache; `h3_block_streaming` / `h3_token_reduction` fail loud until implemented. Fast checks: `make test-h3-parity`; E2E smoke: `make bench-h3-smoke` (installed bundle). Minimum clip ~5s (`num_frames` ≥ 125 @24fps, 17n+5 rule).
- **Assistant LLM memory** — separate **`backend_llm`** sidecar (`mlx_vlm.server`), one multimodal model at a time. Settings → **Assistant model cache (minutes)** (`llm_cache_ttl_minutes`, default 30) idle-unloads via `POST /unload`. **Unload after each request** (`llm_unload_each_request`, default off) overrides TTL. **Generation model cache TTL** (`model_cache_ttl_minutes`) applies only to DiT `ModelCache` in danqing-api, not the sidecar.
