...
- **MiniMax H3 (FL2VA)** — `video_pipeline_shape=family_generator` under `backend/engine/families/minimax_h3/`. **PipeNetwork/minimax-h3-mlx** is the parity reference for scheduler, RoPE, and keyframe conditioning (not upstream original weight key names). **Compute-bound**: mlx-q4/q8 saves RAM more than wall time. Turbo: install LoRA **`minimax-h3-turbo-lora:v4-ema`** (larryvrh v4-600 EMA) or set **`h3_quality_preset=draft`** / **`h3_turbo=true`**; 4–8 steps (6–8 recommended). `h3_turbo` conflicts with TeaCache; `h3_block_streaming` / `h3_token_reduction` fail loud until implemented. Fast checks: `make test-h3-parity`; E2E smoke: `make bench-h3-smoke` (installed bundle). Minimum clip ~5s (`num_frames` ≥ 125 @24fps, 17n+5 rule).
