#!/usr/bin/env python3
"""Bump Danmo Make version, summarize commits, optionally commit/tag/push/release.
...
def bump_files(version: str) -> list[Path]:
    ...
    new, n = re.subn(
        rf'(name = "{re.escape(DESKTOP_PKG)}"\n)version = "[^"]*"',
        rf'\1version = "{version}"',
        text,
        count=1,
    )
    if n != 1:
        die(f"failed to bump {DESKTOP_PKG} in {CARGO_LOCK}")
    if new != text:
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Bump Danmo Make version and prepare a tagged release.",
    )
    p.add_argument(
        "version",
        nargs="?",
